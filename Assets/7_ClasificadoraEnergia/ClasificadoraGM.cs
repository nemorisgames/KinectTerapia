...
public class ClasificadoraGM : MonoBehaviour {
	...
	void Update () {
		...
		if(moveRef.position.y <= 0.5f)
			palancaSign = Mathf.Sign(moveRef.position.x - moveRefLast.x);

		float palancaX = 0;
		float palancaZ = 0;
		if(Mathf.Abs(moveRef.position.x) > 0.25f){
			palancaX = Mathf.Clamp(moveRef.position.y,0,2.5f) * palancaSign;
			palancaZ = Mathf.Clamp(moveRef.position.y,0,2.5f);
		}
		...
		if(palanca.position.z >= 2f && openDoor == 0){
			OpenDoor(palanca.position.x);
		}
		else if(palanca.position.z < 0.5f && openDoor != 0){
			OpenDoor();
		}
	}
